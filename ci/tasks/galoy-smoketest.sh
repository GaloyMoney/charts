...
curl --location --request POST "${host}:${port}/graphql"\
 ...
 --data-raw '{"query":"query btcPriceList($range: PriceGraphRange!) { btcPriceList(range: $range) { timestamp price { base offset currencyUnit formattedAmount } } }","variables":{"range":"ONE_DAY"}}' > response.json
